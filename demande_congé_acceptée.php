 ...
 */
    ...
    class demande_congé_acceptéePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Demandes de congé acceptées');
            $this->SetMenuLabel('Demandes de congé acceptées');
    
            $this->dataset = new TableDataset(
                MySqlIConnectionFactory::getInstance(),
                GetConnectionOptions(),
                '`demande_congé_acceptée`');
        ...
        protected function DoPrepare() {
            if (GetApplication()->IsGETValueSet('demandeID') && GetApplication()->IsGETValueSet('status')) {
                ...
                $sql = "UPDATE demande_congé_acceptée SET `Status`='$stat' WHERE demande_id='$id'";
        ...
        { 
            if ($fieldName === 'Status') {
                if ($fieldData == 'Accéptée') {
                   $customText = '<span class="status-cell green">' . $fieldData . '</span>';
                   $dataAttributes = sprintf('data-demande_id="%s" data-user_id="%s" data-autorisation="%s"', $rowData['demande_id'], $rowData['user_id'], $rowData['Autorisation de sortie du territoire national']);
                   $customText .= '<button class="btn btn-default inline-button" ' . 
                       $dataAttributes . ' style="align-text: center; margin-left: 15px;" onclick="handleDownloadAttestationDecision(this)">Télécharger la décision</button>';
                   if ($rowData['Date de départ'] <= date('Y-m-d') && date('Y-m-d') < $rowData['Date de retour']) {
                      ...
                      $customText .= '<button class="btn btn-default inline-button annuler-button" ' . 
                           $dataAttributes . ' style="align-text: center; margin-left: 15px;">Annuler la demande</button>';
                   }   
                } else if ($fieldData == 'Annulée') {
    ...
    try
    {
        $Page = new demande_congé_acceptéePage("demande_congé_acceptée", "demande_congé_acceptée.php", GetCurrentUserPermissionsForPage("demande_congé_acceptée"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("demande_congé_acceptée"));
